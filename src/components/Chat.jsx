...
const Chat = () => {
  ...
  return (
    <div className='w-full h-screen bg-half-transparent fixed top-0 right-0 z-10'>
      ...
      <div className='nav-item absolute right-5 md:right-52 top-16 bg-white dark:bg-[#42464D] p-8 rounded-lg w-96'>
        ...
        <div className='mt-5'>
          {chatData.map((item, index) => (
            <div key={index} className="flex items-center gap-5 border-b-1 p-3 border-color leading-8 cursor-pointer">
              <div className='relative'>
                <img src={item.image} alt={item.message} className="rounded-full h-10 w-10" />
                <span style={{ background: currentColor }} className='absolute inline-flex rounded-full h-2 w-2 right-0 -top-1'/>
              </div>
              <div>
